...
namespace DNTFrameworkCore.Application.Navigation
{
    public static class MenuItemDefinitionExtensions
    {
        ...
        {
            var menuItem = menuItems.FindMenuItem(menuItemName);
        ...
        {
            var menuItem = menuItems.FindMenuItem(menuItemName);
            var targetMenuItem = menuItems.FindMenuItem(targetMenuItemName);
            menuItems.Remove(menuItem);
        ...
        private static MenuItemDefinition FindMenuItem(this IEnumerable<MenuItemDefinition> menuItems, string menuItemName)
        {
            var menuItem = menuItems.FirstOrDefault(i => i.Name == menuItemName);
            if (menuItem == null)
            {
                throw new FrameworkException("Can not find menu item: " + menuItemName);
            }
