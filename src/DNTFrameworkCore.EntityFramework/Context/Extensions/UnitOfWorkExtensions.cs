...
namespace DNTFrameworkCore.EntityFramework.Context.Extensions
{
    ...
    {
        public static void ApplyChanges(this IUnitOfWork uow, IEnumerable<IHaveTrackingState> roots)
        {
            foreach (var root in roots)
                uow.ApplyChanges(root);
        }

        public static void AcceptChanges(this IUnitOfWork uow, IEnumerable<IHaveTrackingState> roots)
        {
            foreach (var root in roots)
